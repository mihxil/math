...
public  class Solver<E extends RingElement<E>> {
    ...
    native void callBack(long current, long total, Expression<E> expression);

    @SafeVarargs
    public final Stream<Expression<E>> stream(E... set) {
        PermutationGroup permutations = PermutationGroup.ofDegree(set.length);
        long total = permutations.getCardinality().getValue().longValue() * AST.streamSize(set.length, OPERATORS.size()).longValue();
        return permutations.stream()
            ...
            .distinct()
            .peek(e ->
                callBack(tries.getAndIncrement(), total, e)
            );
    }
