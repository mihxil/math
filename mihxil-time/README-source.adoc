...
This module provides things like `org.meeuw.time.UcertainInstant` and `org.meeuw.time.UncertainDuration', which are very similar to their `java.time` equivalents, but implement also `UncertainNumber`, so basically represents temporary objects _with an uncertainty_ in their value.
...
WellknownIrregularHolidaySearcher:: Some (American) events that do not occur every year, like "Election day"
...
- DynamicDateTime always parses to one ZonedDateTime, or gives an exception
