 ...
 * a fixed number of {@link BaseUnit}s, one for each possible {@link SIDimension}.
 */
public interface BaseUnit<D extends Dimension> extends Unit<D> {

    SystemOfMeasurements<D> getSystem();

    SIDimension getDimension();

    @Override
    default List<Quantity> getQuantities() {
        DimensionalAnalysis<D> analysis = new DimensionalAnalysis<>(DimensionExponent.of(getDimension(), 1));
    ...
    @Override
    default DimensionalAnalysis<D> getDimensions() {
        return DimensionalAnalysis.of(getDimension());
    }

    @Override
    default Units<D> times(Units<D> multiplier) {
