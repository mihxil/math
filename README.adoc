...
The field of real numbers. Backed by java primitive `double`s. A `RealNumber` is also 'uncertain', which is used to keep track of rounding errors.
...
The field of reals numbers, but backed by java's `BigDecimal'. This means that it support arbitrary precision, but, since this still
...
Also, division is exact in this field, so this does not implement `UncertainNumber'. The cardinality is countable so this does implement `Streamable`.
...
Actually one of the simplest algebraic object you can thing of are the strings. The form an additive monoid.
